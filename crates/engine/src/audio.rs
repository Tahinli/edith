...
impl AudioSession {
    ...
    /// `Ok(None)` means no audio track — a silent file, which import pairs
    /// only with other silent files.
    ...
    pub fn copy_multi_segments(
        ...
        let Some(track) = copy_track(path)? else {
...
fn copy_track(path: &Path) -> crate::Result<Option<AacTrack>> {
