...
fn open_hw(path: &PathBuf) -> Option<HwSession> {
...
fn run_hw(mut hw: HwSession, tx: &SyncSender<Frame>) -> bool {
    let mut index = 0u32;
    loop {
        match hw.next_frame() {
...
fn run(mut demuxer: Demuxer, tx: SyncSender<Frame>) {
