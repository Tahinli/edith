...
impl EncPictureParameterBufferVP9 {
    ...
    pub fn new(
        ...
        Self(Box::new(bindings::VAEncPictureParameterBufferVP9 {
            ...
            number_skip_frames,
            skip_frames_size,
            // LOCAL PATCH (see /Cargo.toml [patch.crates-io]): libva 1.23 carved
            // `seg_id_block_size` + `va_reserved8` out of this struct's reserved
            // space, so an exhaustive literal no longer compiles. Upstream is
            // only aware of libva <= 1.21.
            ..Default::default()
        }))
    }
