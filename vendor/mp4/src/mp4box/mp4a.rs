...
impl<R: Read + Seek> ReadBox<&mut R> for Mp4aBox {
    fn read_box(reader: &mut R, size: u64) -> Result<Self> {
        ...
        // ISO 14496-12 calls the next 8 bytes reserved; QuickTime (which is
        // what a `qt  ` brand from Resolve, Final Cut, ffmpeg `-f mov` writes)
        // reads them as version(2) revision(2) vendor(4). Version 1 appends a
        // 16-byte compression block after samplerate; version 2 replaces
        // channelcount/samplesize/samplerate with a 36-byte block of its own.
        // Both then wrap the `esds` in a `wave` atom, which the stock parser
        // walked straight past, reporting an AAC track with no `esds`.
        let version = reader.read_u16::<BigEndian>()?;
        reader.read_u16::<BigEndian>()?; // revision
        reader.read_u32::<BigEndian>()?; // vendor
        let mut channelcount = reader.read_u16::<BigEndian>()?;
        let samplesize = reader.read_u16::<BigEndian>()?;
        reader.read_u32::<BigEndian>()?; // compression_id + packet_size
        let mut samplerate = FixedPointU16::new_raw(reader.read_u32::<BigEndian>()?);
        match version {
            1 => skip_bytes(reader, 16)?,
            2 => {
                reader.read_u32::<BigEndian>()?; // sizeOfStructOnly
                let rate = f64::from_bits(reader.read_u64::<BigEndian>()?);
                let channels = reader.read_u32::<BigEndian>()?;
                skip_bytes(reader, 20)?; // always7F000000 .. constLPCMFramesPerAudioPacket
                if (1.0..=f64::from(u16::MAX)).contains(&rate) {
                    samplerate = FixedPointU16::new(rate as u16);
                }
                if let Ok(c) = u16::try_from(channels) {
                    channelcount = c;
                }
            }
            _ => {}
        ...
        let esds = read_esds(reader, start + size)?;
...
/// The first `esds` among the child boxes up to `end`, looking inside a
/// QuickTime `wave` wrapper when one is met. Leaves the reader at `end`.
fn read_esds<R: Read + Seek>(reader: &mut R, end: u64) -> Result<Option<EsdsBox>> {
    let mut esds = None;
    while reader.stream_position()? + HEADER_SIZE <= end {
        let header = BoxHeader::read(reader)?;
        let BoxHeader { name, size: s } = header;
        let here = box_start(reader)?;
        if s == 0 || here + s > end {
            // A `wave` atom ends with an 8-byte terminator whose size is 0;
            // anything else claiming to run past its parent is malformed.
            break;
        }
        if esds.is_none() {
            if name == BoxType::EsdsBox {
                ...
                esds = read_esds(reader, here + s)?;
            }
        }
        skip_bytes_to(reader, here + s)?;
    }
